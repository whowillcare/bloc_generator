...
class Vars:
    ...
    pattern = r'^(?:(?P<clsname>(?P<cls>[\w<,>]+)(?P<optional>\??))\s+)(?P<name>\w+)(?P<value>[\s\S]*)?$'
...
def state_gen(args, data=None):
    fields = shared_fields({
        ...
        'name': None,
        'jsonConverter': '',
        'extraKeys' : [], # where user can including extra keys toJson that need to be defined in .c extracode extension
        'props': [],
    ...
    if args.useJson and args.extraKeys:
        fact = re.sub(r';\s*$','\n..addAll({%s});\n'% (',\n'.join(["'{0}':{0}".format(b) for b in args.extraKeys])) , fact)
    copyWithArgs = []
    copyWithBody = []
    props = []
    init = '%clsname init() {\n   return %clsname();\n  }'.replace('%clsname',
